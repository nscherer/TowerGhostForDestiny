		...
		return function(item){
			...
			if (info.bucketTypeHash in DestinyBucketTypes){
				...
				if ( (DestinyWeaponPieces.indexOf(itemObject.bucketType) > -1 || DestinyArmorPieces.indexOf(itemObject.bucketType) > -1) && item.perks.length > 0 ){
					...
					if (item.talentGridHash in window._talentGridDefs){
						...
						if (unlockableNodes.length > 0) {
							...
							console.log( count_of_highest );
							itemObject.maxProgress = highestStartsAt + pg.steps[pg.steps.length -1].progressTotal * count_of_highest;
							itemObject.remainingProgress = ((_.max([itemObject.maxProgress - itemObject.currentProgress,0]) / itemObject.maxProgress) * 100).toFixed(0);
